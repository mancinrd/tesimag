...
Again we find that current flows in target before the impact of the bullet, when it is distant from the target (parameter $x_{\text{target}} - x(t_{0,i})$ in table), but current rises more rapidly if the bullet has more energy. Comparing these values with those of set A, we find that for the same voltage peak value, the current peak starts faster and reaches an higher peak value. Again, hypotizing that the bullet motion is correlated with the charge motion, an earlier impact point (as in set B compared to set A) means that we will lose lesser charge during the propagation and measure an higher current value on target.
...
\paragraph{Backstream}
For neon the backstream phenomenon is strong, where the bullet reaches the target we find high luminosity around nozzle exit. This backstream presents itself as zone of glowing gas with large width and near constant height, with a barycenter motion that shift direction towards the electrode or towards the target, with so little motion that is not possible to infer it's dynamics. Once the pulse voltage lowers, also the backstream fade away, as the bullet that impacts on target. 

\begin{figure}
 \centering
 \subfloat[Mean luminosity, set A.]{
    \includegraphics[width=0.43\textwidth]{Images/Shape/neon_a_back_Im.png}
 }
 \hfill
 \subfloat[Mean luminosity,  set B.]{
    \includegraphics[width=0.43\textwidth]{Images/Shape/neon_b_back_Im.png}
 }
 
 \subfloat[Barycenter along x, set A.]{
    \includegraphics[width=0.43\textwidth]{Images/Shape/neon_a_back_xb.png}
 }
 \hfill
 \subfloat[Barycenter along x,  set B.]{
    \includegraphics[width=0.43\textwidth]{Images/Shape/neon_b_back_xb.png}
 }
 
 \subfloat[Diameter along x, set A.]{
    \includegraphics[width=0.43\textwidth]{Images/Shape/neon_a_back_dx.png}
 }
 \hfill
 \subfloat[Diameter along x,  set B.]{
